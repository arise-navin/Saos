...
test('T13 — migration 23 is the latest, and re-running migrations is a no-op', () => {
  ...
  try {
    ...
    // HEALTH CATEGORIES appended migration 30 (health_categories + health_category_rules),
    // and migration 31 renamed them to health_dimensions + health_dimension_rules.
    ...
    assert.equal(db.prepare('PRAGMA user_version').get().user_version, 31,
      'the schema is not at exactly 31 migrations — the shipped ones must be untouched and only the '
